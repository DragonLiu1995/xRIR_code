...
sys.path.append("/pscratch/sd/x/xiuliu/room_rir/treble_multi_room_exp/")
import torch
...
# from utils.spec_utils import Criterion
...
def log_mag_stft_loss(gt_spect=None, pred_spect=None, log_gt_eps=1e-8):
   return torch.mse_loss(pred_spect - torch.log(gt_spect + log_gt_eps))
...
def train(model, train_loader, optimizer, lr_scheduler, epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        # log_stft_loss = F.mse_loss(out_spec.cuda(), torch.log(tgt_spec + 1e-8).permute(0, 2, 3, 1).cuda())
        loss = stft_loss + decay_loss #+ log_stft_loss * 0.05
        print("HHH", stft_loss, decay_loss)
...
def test(model, test_loader, epoch):
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            stft_loss = stft_l1_loss(pred_spect=out_spec.cuda(), gt_spect=tgt_spec.permute(0, 2, 3, 1).cuda())
            # log_stft_loss = F.mse_loss(out_spec.cuda(), torch.log(tgt_spec + 1e-8).permute(0, 2, 3, 1).cuda())
            loss = stft_loss + decay_loss #+ 0.05 * log_stft_loss
            print("HHH", stft_loss, decay_loss)
            test_loss += loss.item()
            cnt += 1
...
if __name__ == "__main__":
    ...
    from sim_to_real.haa_classroom_dataset  import HAA_Classroom_RIR_Dataset
    ...
    num_epoch = 200
    train_dataset = HAA_Classroom_RIR_Dataset(split="train", max_len=9600, num_shot=8)
    ...
    test_dataset = HAA_Classroom_RIR_Dataset(split="test",  max_len=9600, num_shot=8)
    ...
    model.load_state_dict(torch.load("/path/to/finetune_all_rooms/ckpt/xxx.pth", map_location="cpu"))
    model.cuda()
    ...
    for ep in range(0, num_epoch + 1):
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(model.state_dict(), "./ckpt/xRIR_8_shot/{}.pth".format(ep)) 
